...
export const projectSchema = z.object({
  ...
  subcategories: project.subcategories.length > 0 ? project.subcategories : (project.subcategory ? [project.subcategory] : []),
  project_types: project.project_types.length > 0 ? project.project_types : [project.project_type],
  maturity_scores: project.maturity_scores ?? {
    code: { score: project.activity_score, rationale: "Derived from activity metrics." },
    governance: { score: project.community_health.governance_maturity, rationale: "Derived from governance transparency indicators." },
    stewardship: { score: project.sustainability_score, rationale: "Derived from sustainability inputs until manual stewardship data is added." },
    public_sector: { score: Math.min(100, project.deployment_countries.length * 12), rationale: "Estimated from known deployments." },
    accessibility: { score: project.accessibility_score, rationale: "Derived from current accessibility score." },
    sustainability: { score: project.sustainability_score, rationale: "Derived from sustainability score." },
    security: { score: Math.max(0, 100 - project.community_health.security_advisories_open * 15), rationale: "Derived from open security advisories." },
    interoperability: { score: Math.min(100, project.standards_alignment.length * 20 + project.interoperability_frameworks.length * 20), rationale: "Estimated from standards and interoperability frameworks." },
    procurement: { score: Math.min(100, project.deployment_countries.length * 10 + (project.licenses.length > 0 ? 20 : 0)), rationale: "Estimated from deployments and licensing clarity." },
    community: { score: Math.round(project.community_health.issue_closure_rate * 100), rationale: "Derived from closure rate and contributor health." },
    documentation: { score: project.community_health.documentation_quality, rationale: "Derived from documentation quality." }
  },
  accessibility_maturity: project.accessibility_maturity ?? (project.accessibility_score >= 80 ? "leading" : project.accessibility_score >= 65 ? "mature" : project.accessibility_score >= 45 ? "growing" : project.accessibility_score >= 25 ? "emerging" : "nascent"),
  sustainability_maturity: project.sustainability_maturity ?? (project.sustainability_score >= 80 ? "leading" : project.sustainability_score >= 65 ? "mature" : project.sustainability_score >= 45 ? "growing" : project.sustainability_score >= 25 ? "emerging" : "nascent"),
  procurement_readiness: project.procurement_readiness ?? (project.deployment_countries.length >= 8 ? "leading" : project.deployment_countries.length >= 4 ? "mature" : project.deployment_countries.length >= 2 ? "growing" : project.deployment_countries.length >= 1 ? "emerging" : "nascent")
}));
